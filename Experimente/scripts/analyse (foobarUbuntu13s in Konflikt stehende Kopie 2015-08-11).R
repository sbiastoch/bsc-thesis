...
plot_majorites <- function(results, num_syls, folder) {
	...
	stat = rbind(
		majority_correct/nrow(results), 	minority_correct/nrow(results),
		majority_error/nrow(results), 	minority_error/nrow(results)
	)
...
#	total_rows = sum(unlist(errors[1]))
	...
	x <- barplot(stat,horiz=TRUE, col=colors, main=paste(num_syls, 'Silben: Fehler je Modell'))
	y <- 0
	apply(stat, 1, function(row) {
		text(y+(row*0.5), x, labels=row, col="black", cex=0.62)
		y <<- y + row
	})
...
#	colnames(results) = lapply(colnames(results), pretty_name) ### verursacht data too long
	...
	plot_basic_stats(results_errors, num_syls, folder)

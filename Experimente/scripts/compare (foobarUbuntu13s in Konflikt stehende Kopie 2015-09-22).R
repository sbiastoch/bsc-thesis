...
function() {
	...
	counts = counts[,2:7]
	counts = cbind(counts, results)
	colnames(counts) = c(2:7,'result')
...
#	})
	...
	countstats = unlist(lapply(featuresets, function(fs) x[grep(fs,names(x))]), recursive=FALSE)
	plot(counts[1,], countstats[[1]], xlim=c(0,1700), col=rainbow(6), pch=20, ylim=c(35,100), ylab='Prozent korrekt', xlab='Anzahl verschiedener Werte im Featureset')
	...
	points(y=unlist(lapply(countstats, max)), x=unlist(c(counts['all',c(1,2,3,4)], counts['sparse',5], counts['all',6])), pch=4)
...
j48_rules_plot = function(syls, colors) {
	...
	plot(steps,w[,'measureNumLeaves'], axes=0, type='l', xlab='', ylab='', ylim=c(0,1500), lty=3, col=colors, lwd=1.5)
	axis(4, col.lab='darkgray', at=c(0,100,200,300,400,500,600,700,800,900,1000,1100,1200,1300,1400,1500))
	par(new=T)
	plot(steps,w[,'Percent_correct'], type='l', xlab='', ylab='', axes=0, ylim=c(83,96), col=colors, lwd=1.5)
	axis(1, at=steps)
	axis(2, at=c(83,85,87,89,91,93,95,97))
	#par(new=F)
}
par(mar=c(2,2,.1,3))
...
jrip_rules_plot = function(syls, colors) {
	...
	steps = c(2,5,10,15,20,30,50,100)
	plot(steps,w[,'measureNumRules'], axes=0, type='l', xlab='', ylab='', ylim=c(0,60), lty=3, col=colors, lwd=1.5)
	axis(4, col.lab='darkgray', at=c(0,10,20,30,40,50,60))
	par(new=T)
	plot(steps,w[,'Percent_correct'], type='l', xlab='', ylab='', axes=0, ylim=c(75,100), col=colors, lwd=1.5)
	axis(1, at=steps)
	axis(2, at=c(75,80,85,90,95,100))
	#par(new=F)
}
par(mar=c(2,2,.1,3))
